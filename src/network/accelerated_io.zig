...
pub const AcceleratedIO = struct {
    ...
    fn tryInitXdpStandalone(self: *Self) bool {
        ...
        xdp_prog_ptr.* = af_xdp.XdpProgram.initWithoutAttach(
            self.allocator,
            ifindex,
            .skb, // SAFETY: SKB mode to avoid ixgbe driver DMA lockups
    ...
    fn initStandardUdp(self: *Self) !void {
        std.debug.print("[ACCEL-IO] initStandardUdp: creating socket for port {d}\n", .{self.config.bind_port});
        var udp = try socket.UdpSocket.init();
        errdefer udp.deinit();

        if (self.config.bind_port > 0) {
            std.debug.print("[ACCEL-IO] initStandardUdp: binding port {d}...\n", .{self.config.bind_port});
            udp.bindPort(self.config.bind_port) catch |err| {
                std.debug.print("[ACCEL-IO] initStandardUdp: FAILED to bind port {d}: {}\n", .{ self.config.bind_port, err });
                return err;
            };
            std.debug.print("[ACCEL-IO] initStandardUdp: port {d} bound successfully\n", .{self.config.bind_port});
        }
...
pub fn createTvuIOWithQueue(allocator: Allocator, interface: []const u8, port: u16, queue_id: u32) !*AcceleratedIO {
    return AcceleratedIO.init(allocator, .{
        ...
        .zero_copy = false, // copy mode for ixgbe safety
    });
}
