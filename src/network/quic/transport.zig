...
pub const Connection = struct {
    ...
    next_stream_id: u64,
    // Fast path stream reuse by priority
    stream_cache: [5]?*Stream,
    ...
    pub fn init(allocator: Allocator, config: ConnectionConfig) !*Connection {
        ...
        conn.* = .{
            ...
            .next_stream_id = 0,
            .stream_cache = .{ null, null, null, null, null },
    ...
    /// Send a batch of messages on a single stream (coalesced)
    pub fn sendBatch(self: *Connection, msgs: []const Message, priority: Priority) !usize {
        if (!self.connected.load(.acquire)) return error.NotConnected;
        if (msgs.len == 0) return 0;

        self.mutex.lock();
        defer self.mutex.unlock();

        const stream_ptr = try self.getOrCreateStream(priority);

        var sent: usize = 0;
        for (msgs) |msg| {
            const encoded = try self.encodeMessage(msg);
            errdefer self.allocator.free(encoded);
            try stream_ptr.write(encoded);
        ...
        try self.flushStream(stream_ptr);
        return sent;
    }
    ...
    fn sendDatagram(self: *Connection, data: []const u8) !void {
        ...
        const rc = std.posix.system.sendto(self.socket.?, data.ptr, data.len, 0, null, 0);
        if (rc == -1) {
            const err = std.posix.errno(rc);
            switch (err) {
                .CONNREFUSED => {
                    // Peer not listening; treat as transient for QUIC-over-UDP.
                    std.log.debug("[QUIC] Datagram send refused (ECONNREFUSED)", .{});
                    return;
                },
                .HOSTUNREACH, .NETUNREACH => {
                    std.log.debug("[QUIC] Datagram send unreachable: {}", .{err});
                    return;
                },
    ...
    fn getOrCreateStream(self: *Connection, priority: Priority) !*Stream {
        const idx: usize = @intFromEnum(priority);
        var stream: ?*Stream = self.stream_cache[idx];

        if (stream == null or stream.?.state != .open) {
            ...
            while (iter.next()) |s| {
                ...
                new_stream.* = Stream.init(self.allocator, stream_id);
                new_stream.priority = priority;
                new_stream.state = .open;
            ...
            self.stream_cache[idx] = stream;
        } else {
            const stream_ptr = stream.?;
            self.stats.streams_reused += 1;
            std.log.debug("[QUIC] Reused stream for priority {s} (id={d})", .{
                @tagName(priority),
                stream_ptr.id,
            });
        }
    ...
    fn flushStream(self: *Connection, stream: *Stream) !void {
        ...
        const send_data = stream.send_buffer.items;
        var offset: usize = 0;

        while (offset < send_data.len) {
            const chunk_size = @min(MAX_DATAGRAM_SIZE - 16, send_data.len - offset); // 16 bytes for stream frame header
            const chunk = send_data[offset..][0..chunk_size];
            ...
            frame[15] = if (offset + chunk_size >= send_data.len) 1 else 0; // FIN flag
    ...
    fn handleStreamFrame(self: *Connection, frame: []const u8) !void {
        ...
        if (fin) {
            ...
            // Decode all messages in the stream buffer
            try self.decodeMessages(stream.recv_buffer.items);
            stream.recv_buffer.clearRetainingCapacity();
        }
    }

    fn decodeMessages(self: *Connection, data: []const u8) !void {
        var offset: usize = 0;
        while (offset < data.len) {
            if (data.len - offset < WireHeader.SIZE) return error.InvalidMessage;
            const header = WireHeader.decode(data[offset .. offset + WireHeader.SIZE]);
            var msg_len: usize = WireHeader.SIZE + header.length;
            if (header.flags.has_correlation) msg_len += 8;
            if (data.len < offset + msg_len) return error.InvalidMessage;

            const msg = try self.decodeMessage(data[offset .. offset + msg_len]);
            try self.incoming_messages.append(msg);
