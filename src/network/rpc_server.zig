...
pub const RpcHttpServer = struct {
    ...
    fn handleConnection(self: *Self, conn: net.Server.Connection) !void {
        ...
        const header_idx = header_end orelse {
            std.log.warn("RPC missing header end: bytes={d}", .{total_read});
            try self.sendError(conn, 400, "Bad Request");
            return;
        };

        // Parse Content-Length if present and read full body.
        var content_len: usize = 0;
        const header_bytes = buf[0..header_idx];
        var lines = std.mem.splitSequence(u8, header_bytes, "\r\n");
        while (lines.next()) |line| {
            if (std.ascii.startsWithIgnoreCase(line, "content-length:")) {
                const after = line["content-length:".len..];
                const trimmed = std.mem.trim(u8, after, " ");
                content_len = std.fmt.parseInt(usize, trimmed, 10) catch 0;
                break;
            }
        }

        const body_start = header_idx + 4;
        const needed = if (content_len > 0) body_start + content_len else total_read;
        if (content_len > 0 and needed > buf.len) {
            std.log.warn("RPC body too large: need={d} max={d}", .{ needed, buf.len });
            try self.sendError(conn, 413, "Payload Too Large");
            return;
        }
        if (content_len > 0 and needed > total_read and needed <= buf.len) {
        ...
        // Find body
        const body = request_data[body_start..@min(request_data.len, needed)];
        if (content_len > 0 and body.len < content_len) {
            std.log.warn("RPC short body: got={d} need={d}", .{ body.len, content_len });
            try self.sendError(conn, 400, "Bad Request");
            return;
        }
        std.log.info(
            "[RPC] request bytes={d} body_len={d} content_len={d}",
            .{ total_read, body.len, content_len },
        );
    ...
    fn processJsonRpc(self: *Self, body: []const u8) ![]u8 {
        ...
        const method = self.extractMethod(body) orelse {
            std.log.warn("RPC invalid request: body_len={d}", .{body.len});
    ...
    pub fn setAccountsDb(self: *Self, accounts_db: ?*storage.AccountsDb) void {
        self.context.accounts_db = accounts_db;
    }

    pub fn setLedgerDb(self: *Self, ledger_db: ?*storage.LedgerDb) void {
        self.context.ledger_db = ledger_db;
    }

    pub fn setSnapshotManager(self: *Self, snapshot_manager: ?*storage.SnapshotManager) void {
        self.context.snapshot_manager = snapshot_manager;
    }

    pub fn setIdentity(self: *Self, identity: ?[]const u8) void {
        self.context.identity = identity;
    }

    pub fn setVoteAccount(self: *Self, vote_account: ?[]const u8) void {
        self.context.vote_account = vote_account;
    }
