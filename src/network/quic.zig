...
fn logPacket(direction: []const u8, pkt_type: PacketType, pn: u64, payload_len: usize) void {
    std.debug.print("[QUIC] {s} {s} pn={d} payload={d}\n", .{
        direction,
        @tagName(pkt_type),
        pn,
        payload_len,
...
pub const Connection = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, endpoint: *Endpoint, peer_addr: packet.SocketAddr, is_server: bool) !*Self {
        ...
        conn.* = .{
            ...
            .crypto_recv = std.ArrayList(u8).init(allocator),
            .client_hello = null,
    ...
    pub fn send(self: *Self, stream_id: u64, data: []const u8) !void {
        ...
        try self.endpoint.sendStreamData(self, stream_id, data);
    }

    fn appendCryptoData(self: *Self, data: []const u8) !void {
        try self.crypto_recv.appendSlice(data);
        while (self.crypto_recv.items.len >= 4) {
            ...
            std.debug.print("[QUIC] handshake msg type=0x{x} len={d}\n", .{ msg_type, len });
            switch (msg_type) {
...
pub const Endpoint = struct {
    ...
    pub fn connect(self: *Self, peer_addr: packet.SocketAddr) !*Connection {
        ...
        std.debug.print("[QUIC] Created connection: local_cid={x} initial_dcid={x} peer={any}\n", .{
            std.fmt.fmtSliceHexLower(conn.local_cid.slice()),
            std.fmt.fmtSliceHexLower(conn.initial_dcid.slice()),
            peer_addr,
        });
    ...
    fn sendInitialPacket(self: *Self, conn: *Connection) !void {
        ...
        std.debug.print("[QUIC] client_hello len={d} first={x:0>2} {x:0>2} {x:0>2} {x:0>2}\n", .{
            client_hello.len,
            client_hello[0],
            client_hello[1],
            client_hello[2],
    ...
    fn sendStreamData(self: *Self, conn: *Connection, stream_id: u64, data: []const u8) !void {
        if (self.sock == null) return error.NotBound;
        if (!conn.tls.handshake_complete) return error.HandshakeIncomplete;

        var payload: [1500]u8 = undefined;
        var offset: usize = 0;
        payload[offset] = @intFromEnum(FrameType.stream) | 0x02; // STREAM with LEN
        offset += 1;
    ...
    fn sendAck(self: *Self, conn: *Connection, packet_number: u64, pkt_type: PacketType) !void {
        ...
        if (pkt_type == .handshake or pkt_type == .initial) {
            ...
            var ciphertext: [128]u8 = undefined;
            var auth_tag: [16]u8 = undefined;
            try conn.tls.encryptPacket(true, pn, header, payload[0..offset], ciphertext[0..offset], &auth_tag);
            @memcpy(pkt.data[header_offset..][0..offset], ciphertext[0..offset]);
            header_offset += offset;
            @memcpy(pkt.data[header_offset..][0..16], &auth_tag);
            header_offset += 16;
    ...
    fn processLongHeaderPacket(self: *Self, pkt: *const packet.Packet) !void {
        ...
        std.debug.print("[QUIC] processing long header: type={s} dcid={x} scid={x}\n", .{
            @tagName(pkt_type),
            std.fmt.fmtSliceHexLower(dcid.slice()),
            std.fmt.fmtSliceHexLower(scid.slice()),
        });
        ...
        if (conn == null and !self.is_server) {
            ...
            while (conn_iter.next()) |c| {
                if (c.*.initial_dcid.eql(&dcid)) {
                    conn = c.*;
                    std.debug.print("[QUIC] Found connection by initial_dcid match ({s})\n", .{@tagName(pkt_type)});
                    break;
                }
            }
            if (conn == null) {
                std.debug.print("[QUIC] No connection found for dcid: {x} (client mode)\n", .{std.fmt.fmtSliceHexLower(dcid.slice())});
            }
        }
        ...
        if (conn) |c| {
            ...
            const hp_key = switch (pkt_type) {
                .initial => if (c.tls.initial_secrets) |secrets| secrets.server.hp else return,
                .handshake => if (c.tls.handshake_secrets) |secrets| secrets.server.hp else return,
                else => c.tls.server_hp,
            };
            ...
            switch (pkt_type) {
                .initial => {
                    if (c.tls.initial_secrets) |secrets| {
                        const ctx = tls13.AeadContext.init(secrets.server, .TLS_AES_128_GCM_SHA256);
                        ctx.decrypt(pn, header_buf[0 .. pn_offset + pn_len], ciphertext, tag_buf, plaintext_buf[0..ciphertext_len]) catch |err| {
                            std.debug.print("[QUIC] initial decrypt failed pn={d} err={}\n", .{ pn, err });
                            return err;
                        };
                    } else {
                        std.debug.print("[QUIC] missing initial secrets for decrypt\n", .{});
                        return;
                    }
                },
                .handshake => {
                    if (c.tls.handshake_secrets) |secrets| {
                        const ctx = tls13.AeadContext.init(secrets.server, c.tls.cipher_suite);
                        ctx.decrypt(pn, header_buf[0 .. pn_offset + pn_len], ciphertext, tag_buf, plaintext_buf[0..ciphertext_len]) catch |err| {
                            std.debug.print("[QUIC] handshake decrypt failed pn={d} err={}\n", .{ pn, err });
                            return err;
                        };
                    } else {
                        std.debug.print("[QUIC] missing handshake secrets for decrypt\n", .{});
                        return;
                    }
                },
                else => {
                    c.tls.decryptPacket(false, pn, header_buf[0 .. pn_offset + pn_len], ciphertext, tag_buf, plaintext_buf[0..ciphertext_len]) catch |err| {
                        std.debug.print("[QUIC] long header decrypt failed pn={d} err={}\n", .{ pn, err });
                        return err;
                    };
                },
            }
    ...
    fn processShortHeaderPacket(self: *Self, pkt: *const packet.Packet) !void {
        ...
        if (!conn.tls.handshake_complete) {
            std.debug.print("[QUIC] drop short header before handshake\n", .{});
            return;
        }
        ...
        var header_buf: [256]u8 = undefined;
        @memcpy(header_buf[0..header_len], pkt.data[0..header_len]);
        const pn_len = conn.tls.unprotectHeader(false, header_buf[0..header_len], pn_offset, sample);
        if (pn_offset + pn_len > pkt.len) return;
        ...
        var tag_buf: [16]u8 = undefined;
        @memcpy(&tag_buf, tag);
        conn.tls.decryptPacket(false, pn, header_buf[0 .. pn_offset + pn_len], ciphertext, tag_buf, plaintext_buf[0..ciphertext_len]) catch |err| {
            std.debug.print("[QUIC] short header decrypt failed pn={d} err={}\n", .{ pn, err });
            return err;
        };
    ...
    fn processFrames(self: *Self, conn: *Connection, payload: []const u8) !void {
        ...
        while (offset < payload.len) {
            ...
            offset += 1;
            std.debug.print("[QUIC] frame type=0x{x}\n", .{frame_type});
            ...
            switch (frame_type) {
                ...
                @intFromEnum(FrameType.crypto) => {
                    ...
                    std.debug.print("[QUIC] crypto offset={d} len={d}\n", .{ crypto_offset, crypto_len });
                ...
                @intFromEnum(FrameType.connection_close), @intFromEnum(FrameType.connection_close_app) => {
                    ...
                    if (frame_type == @intFromEnum(FrameType.connection_close)) {
                        ...
                        if (offset + reason_len <= payload.len) {
                            ...
                            std.debug.print("[QUIC] close err={d} frame={d} reason={s}\n", .{
                                err_code,
                                frame_code,
                                reason,
                            });
                            ...
                            std.debug.print("[QUIC] close err={d} frame={d} reason=<truncated>\n", .{ err_code, frame_code });
                        }
                    } else {
                        var reason_len: u64 = 0;
                        const reason_len_len = decodeVarInt(payload[offset..], &reason_len) orelse return;
                        ...
                        if (offset + reason_len <= payload.len) {
                            const reason = payload[offset..][0..@intCast(reason_len)];
                            std.debug.print("[QUIC] app-close err={d} reason={s}\n", .{ err_code, reason });
                            offset += @intCast(reason_len);
                        } else {
                            std.debug.print("[QUIC] app-close err={d} reason=<truncated>\n", .{err_code});
                        }
                    }
                    conn.state = .draining;
                    break;
    ...
    pub fn poll(self: *Self) !void {
        ...
        self.flush() catch |err| {
            std.debug.print("[QUIC] poll flush error: {}\n", .{err});
        };
        ...
        const received = self.sock.?.recvBatch(&batch) catch |err| {
            std.debug.print("[QUIC] poll: recvBatch error: {}\n", .{err});
            return;
        };

        if (received > 0) {
            std.debug.print("[QUIC] poll: received {d} packets\n", .{received});
        }
...
pub const QuicClient = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, bind_port: u16, config: Endpoint.EndpointConfig) !*Self {
        ...
        const bind_addr = packet.SocketAddr.ipv4(.{ 0, 0, 0, 0 }, bind_port);
        client.* = .{
            .allocator = allocator,
