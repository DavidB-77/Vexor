...
/// Testnet shred version (updated 2025-01-25 - query with getClusterNodes to verify)
...
pub const GossipService = struct {
    ...
    pub fn start(self: *Self) !void {
        ...
        std.debug.print("[Gossip] Socket fd={d} bound, requested port={d}, actual port={any}\n", .{
            sock.fd, self.config.gossip_port, actual_port,
        });

        if (actual_port) |port| {
            if (port != self.config.gossip_port) {
                std.debug.print("[Gossip] ❌ PORT MISMATCH! Requested {d} but got {d}\n", .{
                    self.config.gossip_port, port,
                });
                return error.PortBindFailed;
            }
        } else {
            std.debug.print("[Gossip] ❌ Could not verify bound port!\n", .{});
            return error.PortBindFailed;
        }
        ...
        std.debug.print("[Gossip] ✅ Gossip service started on port {d} (fd={d})\n", .{
    ...
    fn handlePullRequest(self: *Self, pkt: *const packet.Packet) !void {
        ...
        if (self.modern_contact_info == null and self.legacy_contact_info == null) return;
        ...
        if (self.legacy_contact_info) |*info| {
            info.wallclock = @intCast(now_ms);
        }

        var pkt_out = packet.Packet.init();
        const sender_pubkey = self.identity;

        if (self.legacy_contact_info) |*legacy| {
            var legacy_sig: ?core.Signature = null;
            if (self.keypair) |kp| {
                var signable: [512]u8 = undefined;
                const signable_len = bincode.getLegacyContactInfoSignableData(legacy, &signable) catch 0;
                if (signable_len > 0) {
                    legacy_sig = kp.sign(signable[0..signable_len]);
                }
            }
            const len = bincode.buildPullResponseWithLegacyContactInfo(
                &pkt_out.data,
                sender_pubkey,
                legacy,
                legacy_sig,
            ) catch |err| {
                std.debug.print("[Gossip] Failed to build pull response: {}\n", .{err});
                return;
            };
    ...
    fn sendPullRequest(self: *Self, target: packet.SocketAddr) !void {
        ...
        // Prefer LegacyContactInfo for Agave/localnet compatibility
        if (self.legacy_contact_info) |*legacy| {
            var legacy_sig: ?core.Signature = null;
            if (self.keypair) |kp| {
                var signable: [512]u8 = undefined;
            ...
            const legacy_len = bincode.buildPullRequestWithLegacyContactInfo(
                &pkt.data,
                legacy,
                legacy_sig,
            ) catch |err| {
                std.debug.print("[Gossip] Failed to build legacy pull request: {}\n", .{err});
                return;
            };
            pkt.len = @intCast(legacy_len);
            pkt.src_addr = target;
            _ = try self.sock.?.send(&pkt);
        }

        // Also send modern ContactInfo for Firedancer compatibility
        if (self.modern_contact_info) |*modern| {
            var signature: ?core.Signature = null;
            if (self.keypair) |kp| {
