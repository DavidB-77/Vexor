...
pub fn buildClientHello(
    ...
    try appendExtension(&msg, @intFromEnum(ExtensionType.psk_key_exchange_modes), &[_]u8{
        0x01, // length
        0x01, // psk_dhe_ke
    });
...
pub const TransportParameters = struct {
    ...
    pub fn encode(self: *const TransportParameters, allocator: std.mem.Allocator) ![]u8 {
        ...
        if (self.disable_active_migration) {
            try encodeEmptyParam(&buf, 0x0c);
        }
        try encodeVarInt(&buf, 0x0e, self.active_connection_id_limit);
        try encodeParam(&buf, 0x0f, self.initial_source_cid_len, self.initial_source_cid);

        return buf.toOwnedSlice();
    }

    fn appendVarInt(buf: *std.ArrayList(u8), value: u64) !void {
    ...
    fn encodeParam(buf: *std.ArrayList(u8), id: u64, len: u8, data: ?[20]u8) !void {
        if (data) |d| {
            try appendVarInt(buf, id);
            try appendVarInt(buf, len);
            try buf.appendSlice(d[0..len]);
        }
    }

    fn encodeEmptyParam(buf: *std.ArrayList(u8), id: u64) !void {
        try appendVarInt(buf, id);
        try appendVarInt(buf, 0);
    }

    fn encodeVarInt(buf: *std.ArrayList(u8), id: u64, value: u64) !void {
        var tmp: [8]u8 = undefined;
        ...
        try appendVarInt(buf, id);
        try appendVarInt(buf, value_len);
        try buf.appendSlice(tmp[0..value_len]);
    }
};
