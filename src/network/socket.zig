...
pub const UdpSocket = struct {
    ...
    pub fn setBusyPoll(self: *Self, prefer: bool, busy_poll_us: u32, busy_poll_budget: u32) void {
        ...
        posix.setsockopt(self.fd, posix.SOL.SOCKET, SO_PREFER_BUSY_POLL, &std.mem.toBytes(prefer_val)) catch |err| {
            std.debug.print("Warning: Could not set SO_PREFER_BUSY_POLL: {}\n", .{err});
        };
        ...
        posix.setsockopt(self.fd, posix.SOL.SOCKET, SO_BUSY_POLL, &std.mem.toBytes(poll_us)) catch |err| {
            std.debug.print("Warning: Could not set SO_BUSY_POLL: {}\n", .{err});
        };
        ...
        posix.setsockopt(self.fd, posix.SOL.SOCKET, SO_BUSY_POLL_BUDGET, &std.mem.toBytes(budget_val)) catch |err| {
            std.debug.print("Warning: Could not set SO_BUSY_POLL_BUDGET: {}\n", .{err});
        };
    }
    ...
    pub fn recv(self: *Self, pkt: *packet.Packet) !bool {
        ...
        if (result) |bytes| {
            // std.debug.print("[Socket] recv: got {d} bytes from {any}\n", .{ bytes, src_addr });
            ...
            // std.debug.print("[Socket] recv error: {}\n", .{err});
            return err;
        }
    }
    ...
    fn recvBatchLinux(self: *Self, batch: *packet.PacketBatch) !usize {
        ...
        for (0..batch_size) |i| {
            ...
            iovecs[i] = .{
                .base = &batch.packets[pkt_idx].data,
        ...
        const rc = std.os.linux.syscall5(.recvmmsg, @intCast(self.fd), @intFromPtr(&hdrs), @intCast(batch_size), 0, 0);
        ...
        for (0..received) |i| {
            const pkt_idx = start_idx + i;
            batch.packets[pkt_idx].len = @intCast(hdrs[i].msg_len);
            batch.packets[pkt_idx].src_addr = sockaddrToPacketAddr(&addrs[i]);
    ...
    fn sendBatchLinux(self: *Self, batch: *const packet.PacketBatch) !usize {
        ...
        const rc = std.os.linux.syscall4(.sendmmsg, @intCast(self.fd), @intFromPtr(&hdrs), @intCast(batch_size), 0);
