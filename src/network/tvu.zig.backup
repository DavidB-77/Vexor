...
pub const TvuService = struct {
    ...
    pub const Config = struct {
        ...
        /// Validator identity (32-byte pubkey for repair requests)
    ...
    pub fn requestRepairs(self: *Self, slot: core.Slot, missing_indices: []const u32) !void {
        ...
        for (missing_indices) |shred_idx| {
            ...
            // Build repair request (WindowIndex format)
            // Format: [type: u8] [slot: u64] [shred_index: u64] [nonce: u32] [from: 32]
            pkt.data[0] = 0x00; // WindowIndex repair type
            std.mem.writeInt(u64, pkt.data[1..9], slot, .little);
            std.mem.writeInt(u64, pkt.data[9..17], shred_idx, .little);
            std.mem.writeInt(u32, pkt.data[17..21], @intCast(@as(u32, @truncate(@as(u64, @intCast(std.time.timestamp()))))), .little);
            @memcpy(pkt.data[21..53], &self.config.identity);
            pkt.len = 53;

            // Send to repair peers (try 3 peers for redundancy)
            ...
            // Log first request per batch
            if (shred_idx == missing_indices[0]) {
                std.debug.print("[REPAIR] Sending request for slot {d} idx {d} to {d} peers\n", .{
    ...
    fn getRepairPeers(self: *Self, slot: core.Slot) []const packet.SocketAddr {
        _ = slot;
        
        // Static buffer to hold discovered repair peers (persists across calls)
        const S = struct {
            var repair_peers: [5]packet.SocketAddr = undefined;
        ...
        if (self.gossip_service) |gs| {
            ...
            while (iter.next()) |entry| {
                ...
                if (info.serve_repair_addr.port() > 0) {
                    S.repair_peers[S.peer_count] = info.serve_repair_addr;
                    S.peer_count += 1;
        ...
        const static_repair_peers = &[_]packet.SocketAddr{
            packet.SocketAddr.ipv4(.{ 192, 155, 103, 41 }, 8013),
            packet.SocketAddr.ipv4(.{ 104, 250, 133, 50 }, 8012),
            packet.SocketAddr.ipv4(.{ 147, 28, 169, 89 }, 8013),
        };
