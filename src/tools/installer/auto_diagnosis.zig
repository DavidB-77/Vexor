...
pub const DetectedIssue = struct {
    ...
    diagnosis_output_owned: bool = false,
...
pub const AutoDiagnosis = struct {
    ...
    pub fn deinit(self: *Self) void {
        for (self.detected_issues.items) |detected| {
            if (detected.diagnosis_output_owned) {
                self.allocator.free(detected.diagnosis_output);
            }
        }
