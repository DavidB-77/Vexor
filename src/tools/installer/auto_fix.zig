...
pub const AutoFix = struct {
    ...
    pub fn applyFix(self: *Self, issue: issue_db.KnownIssue) !FixResult {
        ...
        if (self.dry_run) {
            return FixResult{
                .issue_id = issue.id,
                .success = true,
                .output = "[DRY RUN] Would execute fix command",
        ...
        // Execute the fix
        const cmd_result = self.runCommand(fix.command) catch {
            return FixResult{
                .issue_id = issue.id,
                .success = false,
                .output = "",
    ...
    fn createBackup(self: *Self, issue_id: []const u8) !void {
        ...
        const cmd = try std.fmt.allocPrint(self.allocator, "mkdir -p {s}", .{backup_path});
        defer self.allocator.free(cmd);
        _ = self.runCommand(cmd) catch {};
    }
