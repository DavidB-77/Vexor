...
fn queueCompare(context: void, a: QueuedTransaction, b: QueuedTransaction) std.math.Order {
    _ = context;
...
pub const BankingStage = struct {
    ...
    /// Bank mutex (for safe updates from replay thread)
    bank_mutex: std.Thread.Mutex,

    /// Lock-free incoming queue (TPU -> Banking)
    incoming_queue: *IncomingQueue,

    const Self = @This();

    // 16K capacity input buffer
    pub const IncomingQueue = core.lock_free.MPmcQueue(QueuedTransaction, 16384);
    ...
    pub fn init(allocator: std.mem.Allocator, config: BankingConfig) !*Self {
        ...
        const incoming_queue = try allocator.create(IncomingQueue);
        incoming_queue.* = IncomingQueue.init();
        errdefer allocator.destroy(incoming_queue);
    ...
    pub fn deinit(self: *Self) void {
        ...
        self.allocator.destroy(self.incoming_queue);
        self.allocator.destroy(self);
    }

    /// Update the active bank (called from replay thread)
    pub fn setBank(self: *Self, bank: *Bank) void {
        self.bank_mutex.lock();
        defer self.bank_mutex.unlock();
        self.bank = bank;
    }
    ...
    pub fn run(self: *Self) void {
        ...
        while (self.running.load(.acquire)) {
            ...
            while (self.incoming_queue.pop()) |queued| {
                // Determine source (TPU/Vote)
                // We assume TPU incoming.
                if (queued.source == .tpu_vote) {
