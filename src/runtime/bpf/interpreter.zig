...
/// Forward declaration for syscalls
...
pub const VmContext = struct {
    ...
    /// Syscall handlers
    syscall_handlers: std.AutoHashMap(u32, SyscallFn),
    /// Is running
    running: bool,
    /// Allocator
    allocator: Allocator,
    /// Invoke context for syscall access to runtime state
    invoke_context: ?*syscalls.InvokeContext,
    ...
    pub fn init(allocator: Allocator, bytecode: []const u8, rodata: []const u8, heap_size: usize) !VmContext {
        ...
        var ctx = VmContext{
            ...
            .syscall_handlers = std.AutoHashMap(u32, SyscallFn).init(allocator),
            .running = true,
            .allocator = allocator,
            .invoke_context = null,
        };
    ...
    pub fn deinit(self: *VmContext) void {
        ...
        self.syscall_handlers.deinit();
    }

    pub fn registerSyscall(self: *VmContext, id: u32, handler: SyscallFn) !void {
        try self.syscall_handlers.put(id, handler);
    }

    /// Set the invoke context for syscall access
    pub fn setInvokeContext(self: *VmContext, ctx: *syscalls.InvokeContext) void {
        self.invoke_context = ctx;
    }
};
...
pub const BpfVm = struct {
    ...
    fn executeCall(ctx: *VmContext, insn: BpfInstruction) VmError!void {
        ...
        if (call_type == 0) {
            ...
            const handler = ctx.syscall_handlers.get(syscall_id) orelse return VmError.InvalidSyscall;
