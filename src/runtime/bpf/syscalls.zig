...
/// Invoke context for syscalls - provides runtime state to BPF programs
pub const InvokeContext = struct {
    ...
    /// Initialize with runtime state (slot, epoch, etc.)
    pub fn initWithState(
        ...
        return .{
            ...
            .slot = slot,
            .epoch = epoch,
            .epoch_start_timestamp = unix_timestamp, // Simplified
...
fn syscallGetClockSysvar(ctx: *VmContext, clock_ptr: u64, _: u64, _: u64, _: u64, _: u64) VmError!u64 {
    // Get Clock sysvar
    ...
    // Get values from invoke context if available
    if (ctx.invoke_context) |invoke| {
        clock.* = .{
            .slot = invoke.slot,
            .epoch_start_timestamp = invoke.epoch_start_timestamp,
            .epoch = invoke.epoch,
            .leader_schedule_epoch = invoke.epoch + 1,
            .unix_timestamp = invoke.unix_timestamp,
        };
    } else {
