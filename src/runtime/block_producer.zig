...
pub const Entry = struct {
    ...
    pub fn serialize(self: *const Self, writer: anytype) !void {
        ...
        for (self.transactions) |tx| {
            ...
            // 3. Message (already serialized in Transaction struct)
...
test "block producer entry serialization" {
    const allocator = std.testing.allocator;
    ...
    // Check header
    // num_hashes (8) + hash (32) + num_tx (8) = 48 bytes
    try std.testing.expectEqual(@as(usize, 48), buf.items.len);

    // Test with one transaction
    const sig_data = [_]u8{0x11} ** 64;
    const signature = core.Signature{ .data = sig_data };
    const signatures = [_]core.Signature{signature};
    // Compact-u16 for 1 is 0x01
    ...
    const tx = Transaction{
        ...
        .message = &message,
        .recent_blockhash = undefined,
        .compute_unit_limit = 0,
        .compute_unit_price = 0,
        .account_keys = &[_]core.Pubkey{},
        .account_writability = &[_]bool{},
        .instructions = &[_]bank_mod.Instruction{},
    };

    entry.transactions = &[_]Transaction{tx};
    buf.clearRetainingCapacity();
    try entry.serialize(buf.writer());

    // Header (48) + sig_count_compact (1) + sigs (64) + msg (3) = 116 bytes
    try std.testing.expectEqual(@as(usize, 116), buf.items.len);

    // Verify content
    const data = buf.items;
    // Check sig count (compact u16 for 1 is 0x01)
    try std.testing.expectEqual(data[48], 0x01);
    // Check signature
    try std.testing.expectEqualSlices(u8, &sig_data, data[49..113]);
    // Check message
    try std.testing.expectEqualSlices(u8, &message, data[113..116]);
}
