...
pub const WebSocketServer = struct {
    ...
    fn handleNewConnection(self: *WebSocketServer, stream: std.net.Stream) !void {
        ...
        // ✅ THREAD LEAK FIX: Properly detach thread to allow OS cleanup
        std.debug.print("[WS-SERVER] Spawning handler thread for connection {d}\n", .{id});
        const handler_thread = std.Thread.spawn(.{}, handleConnection, .{ self, conn }) catch |err| {
            std.log.warn("Failed to spawn connection handler: {}", .{err});
            self.removeConnection(id);
            return;
        };
        // Detach immediately - thread will clean up when handleConnection returns
