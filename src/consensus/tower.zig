...
pub const TowerBft = struct {
    ...
    pub const VoteState = struct {
        ...
        pub fn recordVote(self: *VoteState, slot: core.Slot) void {
            ...
            // Use saturating add to prevent overflow in ReleaseSafe
            for (self.votes.slice()) |*lockout| {
                lockout.confirmation_count = lockout.confirmation_count +| 1;
            }
    ...
    pub fn recordExternalVote(self: *Self, external_vote: *const Vote) !void {
        // Find the voter pubkey from the signature (in a real implementation)
        // For now, if we don't have the explicit voter, we use a placeholder
        // or expect the caller to pass it.
        // Solana usually passes the voter pubkey alongside the vote in Gossip.

        // TODO: Pass voter pubkey explicitly to this function
        const voter = core.Pubkey{ .data = [_]u8{0} ** 32 };

        if (self.fork_choice) |fc| {
            try fc.onVoteWithVoter(external_vote, voter);
        }
    }
    ...
    pub fn saveToDisk(self: *const Self, path: []const u8) !void {
        ...
        // Magic + version
        try writer.writeAll("VXR1"); // Vexor tower v1
        try writer.writeInt(u32, 1, .little);

        // Core state
        try writer.writeInt(u64, self.last_vote_slot, .little);
        try writer.writeInt(u64, self.vote_state.root_slot orelse 0, .little);
        try writer.writeInt(u8, if (self.vote_state.root_slot != null) 1 else 0, .little);

        // Lockout stack
        const num_lockouts: u32 = @intCast(self.vote_state.votes.len);
        try writer.writeInt(u32, num_lockouts, .little);
        for (self.vote_state.votes.slice()) |lockout| {
            try writer.writeInt(u64, lockout.slot, .little);
            try writer.writeInt(u32, lockout.confirmation_count, .little);
        }

        // Credits
        try writer.writeInt(u64, self.vote_state.credits, .little);
        try writer.writeInt(u64, self.vote_state.epoch, .little);

        std.log.info("[Tower] Saved state to {s}: last_vote={d} root={?d} lockouts={d}", .{
            path, self.last_vote_slot, self.vote_state.root_slot, num_lockouts,
        });
    }
    ...
    pub fn loadFromDisk(self: *Self, path: []const u8) !void {
        const file = std.fs.cwd().openFile(path, .{}) catch |err| {
            if (err == error.FileNotFound) {
                std.log.info("[Tower] No saved state at {s}, starting fresh", .{path});
                return;
            }
            return err;
        };
        ...
        if (!std.mem.eql(u8, &magic, "VXR1")) {
            std.log.warn("[Tower] Invalid magic in {s}, starting fresh", .{path});
            return;
        }

        const version = try reader.readInt(u32, .little);
        if (version != 1) {
            std.log.warn("[Tower] Unknown version {d} in {s}, starting fresh", .{ version, path });
            return;
        }

        // Core state
        self.last_vote_slot = try reader.readInt(u64, .little);
        const root_slot_val = try reader.readInt(u64, .little);
        const has_root = try reader.readInt(u8, .little);
        self.vote_state.root_slot = if (has_root != 0) root_slot_val else null;

        // Lockout stack
        const num_lockouts = try reader.readInt(u32, .little);
        self.vote_state.votes = std.BoundedArray(Lockout, MAX_LOCKOUT_HISTORY).init(0) catch unreachable;
        var i: u32 = 0;
        while (i < num_lockouts) : (i += 1) {
            const slot = try reader.readInt(u64, .little);
            const conf = try reader.readInt(u32, .little);
            self.vote_state.votes.append(.{
                .slot = slot,
        ...
        // Credits
        self.vote_state.credits = reader.readInt(u64, .little) catch 0;
        self.vote_state.epoch = reader.readInt(u64, .little) catch 0;

        std.log.info("[Tower] Loaded state from {s}: last_vote={d} root={?d} lockouts={d}", .{
            path, self.last_vote_slot, self.vote_state.root_slot, self.vote_state.votes.len,
        });
    }
};
