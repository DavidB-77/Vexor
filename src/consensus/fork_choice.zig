...
pub const ForkChoice = struct {
    ...
    /// Update all voter stakes from a map (usually from Bank)
    pub fn setVoterStakes(self: *Self, stakes: *const std.AutoHashMap(core.Pubkey, u64)) !void {
        self.voter_stakes.clearRetainingCapacity();
        var total_active: u64 = 0;

        var it = stakes.iterator();
        while (it.next()) |entry| {
            try self.voter_stakes.put(entry.key_ptr.*, entry.value_ptr.*);
            total_active = std.math.add(u64, total_active, entry.value_ptr.*) catch std.math.maxInt(u64);
        }

        self.setTotalStake(total_active);
    }
    ...
    pub fn onVoteWithVoter(self: *Self, vote_info: *const Vote, voter: core.Pubkey) !void {
        ...
        if (self.forks.getPtr(vote_info.slot)) |node| {
            node.direct_stake = std.math.add(u64, node.direct_stake, stake) catch std.math.maxInt(u64);
        }
        ...
        entry.value_ptr.* = std.math.add(u64, entry.value_ptr.*, stake) catch std.math.maxInt(u64);
    ...
    fn propagateStake(self: *Self, slot: core.Slot, stake: u64) !void {
        ...
        while (true) {
            if (self.forks.getPtr(current)) |node| {
                ...
                node.cumulative_stake = std.math.add(u64, node.cumulative_stake, stake) catch std.math.maxInt(u64);
...
test "fork choice on vote" {
    ...
    try fc.addFork(0, null, core.Hash.ZERO);
    ...
    try fc.onVote(&vote1);
    try std.testing.expectEqual(@as(?core.Slot, 1), fc.bestSlot());
}
