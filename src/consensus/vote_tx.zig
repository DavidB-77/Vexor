...
pub const VoteTransactionBuilder = struct {
    ...
    pub fn buildVoteTransaction(
        ...
        // Lockout slots (compact-u16)
        try writeCompactU16(&ix_data, @intCast(votes.len));
        for (votes) |v| {
    ...
    pub fn signAndSerialize(
        ...
        try message.append(3); // num_readonly_unsigned_accounts (clock, slot hashes, vote program)

        // Account keys (compact-u16 length)
        try writeCompactU16(&message, 5);
        ...
        try message.appendSlice(&SYSVAR_CLOCK_ID);

        // 3: Slot hashes sysvar (readonly)
        try message.appendSlice(&SYSVAR_SLOT_HASHES_ID);

        // 4: Vote program (readonly)
        try message.appendSlice(&VOTE_PROGRAM_ID);

        // Recent blockhash
        try message.appendSlice(&tx.recent_blockhash.data);

        // Instructions (compact-u16 length)
        try writeCompactU16(&message, 1);

        // Instruction
        try message.append(4); // program_id_index = vote program
        // num accounts (compact-u16)
        try writeCompactU16(&message, 4);
        try message.append(1); // vote account index
        try message.append(3); // slot hashes sysvar
        try message.append(2); // clock sysvar
        try message.append(0); // authority index

        // Instruction data
        const data_len: u16 = @intCast(tx.instruction_data.len);
        try writeCompactU16(&message, data_len);
        try message.appendSlice(tx.instruction_data);

        // Now build full transaction

        // Signature count (compact-u16)
        try writeCompactU16(&result, 2);
...
fn writeCompactU16(data: *std.ArrayList(u8), value: u16) !void {
    if (value < 0x80) {
