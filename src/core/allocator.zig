...
pub const MmapAllocator = struct {
    ...
    /// If expected_size is provided and non-zero, use it instead of stat.size
    /// This prevents race conditions where file size isn't reported correctly after resize
    pub fn mapFile(self: *Self, path: []const u8, writable: bool) ![]align(std.mem.page_size) u8 {
        return self.mapFileWithSize(path, writable, 0);
    }
    
    /// Map a file into memory with explicit size
    pub fn mapFileWithSize(self: *Self, path: []const u8, writable: bool, expected_size: u64) ![]align(std.mem.page_size) u8 {
        ...
        // Use expected_size if provided and larger than stat.size
        // This handles the case where setEndPos was called but stat doesn't reflect it yet
        const size: usize = if (expected_size > 0 and expected_size > stat.size) 
        ...
        const prot: u32 = @as(u32, std.posix.PROT.READ) |
            (if (writable) @as(u32, std.posix.PROT.WRITE) else 0);
