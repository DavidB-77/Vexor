...
pub const Blockstore = struct {
    ...
    /// Get a coding shred
    pub fn getCodingShred(self: *Self, slot: core.Slot, index: u32) ?[]const u8 {
        return self.coding_shreds.get(slot, index);
    }
...
pub const SlotMeta = struct {
    ...
    /// Block hash for this slot (for ancestor hashes repair)
    blockhash: ?[32]u8,
