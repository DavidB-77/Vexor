...
pub const vexstore = @import("vexstore.zig");
pub const bench_storage = @import("bench_storage.zig");
pub const speedb = @import("speedb.zig");
...
pub const VexStore = vexstore.VexStore;
pub const SpeedbStore = speedb.SpeedbStore;
...
pub const StorageManager = struct {
    ...
    async_io_manager: ?*AsyncIoManager = null,
    ...
    pub fn init(allocator: std.mem.Allocator, config: anytype) !*Self {
        ...
        // Initialize Async I/O if enabled in config
        var async_mgr: ?*AsyncIoManager = null;
        const T = @TypeOf(config);
        const PtrInfo = @typeInfo(T);
        const ConfigType = if (PtrInfo == .Pointer) PtrInfo.Pointer.child else T;

        if (@hasField(ConfigType, "enable_async_io") and config.enable_async_io) {
            async_mgr = try AsyncIoManager.init(allocator, .{});
        }

        manager.* = .{
            .allocator = allocator,
            // Pass async manager to AccountsDb
            .accounts_db = try AccountsDb.init(allocator, config.accounts_path, async_mgr),
    ...
    pub fn deinit(self: *Self) void {
        ...
        if (self.async_io_manager) |aio| aio.deinit();
