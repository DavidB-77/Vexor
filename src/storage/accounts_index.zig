...
pub const AccountsIndex = struct {
    ...
    /// Batch entry for atomic updates
    ...
    /// Batch upsert for atomic updates
    /// Holds index_lock for the entire duration
    pub fn upsertBatch(self: *Self, entries: []const BatchEntry) !void {
        // Track which entries need program index updates
        var new_inserts = try std.ArrayList(usize).initCapacity(self.allocator, entries.len);
        defer new_inserts.deinit();

        self.index_lock.lock();
        defer self.index_lock.unlock();

        for (entries, 0..) |entry, i| {
            // Pre-allocate new entry
            const new_entry = try self.allocator.create(IndexEntry);
            new_entry.* = IndexEntry{
                .pubkey = entry.pubkey,
            ...
            if (self.index.getPtr(entry.pubkey)) |existing_ptr| {
                const existing = existing_ptr.*;
                new_entry.previous = existing;
                existing_ptr.* = new_entry;
                _ = @atomicRmw(u64, &self.stats.updates, .Add, 1, .monotonic);
            } else {
                try self.index.put(entry.pubkey, new_entry);
                _ = @atomicRmw(u64, &self.stats.inserts, .Add, 1, .monotonic);
                try new_inserts.append(i);
            }

            if (entry.location.slot > self.latest_slot) {
                self.latest_slot = entry.location.slot;
            }
        }

        // Update program index for new entries
        // Safe to acquire program_lock while holding index_lock
        if (new_inserts.items.len > 0) {
            self.program_lock.lock();
            defer self.program_lock.unlock();

            for (new_inserts.items) |idx| {
                const entry = entries[idx];
                if (self.program_index.getPtr(entry.owner)) |list| {
                    try list.append(entry.pubkey);
                } else {
                    var list = std.ArrayList(Pubkey).init(self.allocator);
                    try list.append(entry.pubkey);
                    try self.program_index.put(entry.owner, list);
                }
            }
        }
    }
