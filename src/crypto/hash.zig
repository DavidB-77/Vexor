...
pub const Sha256 = struct {
    ...
    pub fn final(self: *Sha256) core.Hash {
        return core.Hash{ .data = self.hasher.finalResult() };
    }
...
test "sha256 oneshot" {
    ...
    try std.testing.expect(hash.data[0] != 0);
}
...
test "sha256 incremental" {
    ...
    try std.testing.expectEqualSlices(u8, &expected.data, &hash.data);
}
