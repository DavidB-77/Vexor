...
pub fn build(b: *std.Build) void {
    ...
    const rocksdb_dep = b.dependency("rocksdb-zig", .{});
    const rocksdb_bindings = rocksdb_dep.module("bindings");
    ...
    const auto_optimize_enabled = b.option(bool, "auto_optimize", "Enable auto-optimizer at startup") orelse false;
    const masque_enabled = b.option(bool, "masque", "Enable MASQUE protocol for QUIC proxying") orelse false;
    const vexstore_shadow_enabled = b.option(bool, "vexstore_shadow", "Enable VexStore shadow writes for AccountsDb") orelse false;
    ...
    options.addOption(bool, "vexstore_shadow_enabled", vexstore_shadow_enabled);
    ...
    exe.root_module.addImport("rocksdb_zig", rocksdb_bindings);
    ...
    if (af_xdp_enabled and target.result.os.tag == .linux) {
        ...
        const bpf_compile_step = b.addSystemCommand(&.{
            ...
            "-I",
            b.path("src/network/af_xdp/bpf").getPath(b),
        });
    ...
    main_tests.root_module.addImport("rocksdb_zig", rocksdb_bindings);
    ...
    bench_exe.root_module.addImport("rocksdb_zig", rocksdb_bindings);
    bench_exe.linkLibC();
    ...
    const sim_exe = b.addExecutable(.{
        .name = "vexor-sim",
        .root_source_file = b.path("live_cluster_sim.zig"),
        .target = target,
        .optimize = optimize,
    });
    sim_exe.root_module.addOptions("build_options", options);
    sim_exe.linkLibC();
    sim_exe.root_module.addImport("rocksdb_zig", rocksdb_bindings);

    const sim_cmd = b.addRunArtifact(sim_exe);
    const sim_step = b.step("sim", "Run live cluster simulation");
